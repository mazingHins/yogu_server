...
public final class ThreadLocalContext {
	...
	public static final String REQ_HEADER_PARAM_LANG = "REQ_HEADER_PARAM_LANG";// 多语言 lang 参数
	...
	// public static void cleanPublic() {
	// publicLocal.get().clear();
	// publicLocal.remove();
	// }
	 ...
	 *
	 * <p>
	 * <strong> 注：线程结束后 value 不会被销毁，一直存在于内存中！如果下一个请求使用同样的线程 ID，使用的是同一个对象！ </strong>
	 * </p>
	 *
	 */
//	@Deprecated
	...
	/**
	 * 设置对象值，如果null==value则 表示删除value <strong>该 value 只在线程内有效，线程运行完毕即销毁。</strong>
	 */
	...
	// private static class ThreadClean extends Thread {
	//
	// private WeakReference<Thread> currentThreadRef;
	//
	// private ThreadClean(Thread currentThread) {
	// currentThreadRef = new WeakReference<Thread>(currentThread);
	// }
	//
	// /*
	// * （非 Javadoc）
	...
	// public void run() {
	// Thread thread;
	// if (null == currentThreadRef || null == (thread = currentThreadRef.get()))
	// return;
	//
	// // thread > clean public
	// }
