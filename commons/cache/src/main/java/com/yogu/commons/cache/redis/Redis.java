...
/**
 * redis 接口, overrid只是为了显示参数注释
 * @author hemeng
 *
 */
public interface Redis extends redis.clients.jedis.JedisCommands {

	public static final int defTimeout = BaseCacheService.DEF_EXP;

	/** 获取资源（getResource()）方法，警告的阀值（毫秒） */
	public static final long getResourceWarnTime = 30;

	/** 缓存一个对象，默认时间30天：BaseCacheService.DEF_EXP */
	...
	/**
	 * 返回host(使用多实例的redis需要指定key，单个实例的redis传入null)
	 * @param key 
	 * @return
	 */
	...
	/**
	 * 返回端口(使用多实例的redis需要指定key，单个实例的redis传入null)
	 * @param key 
	 * @return
	 */
