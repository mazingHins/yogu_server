...
public abstract class JedisClusterCommand<T> {
  ...
  public T runBinary(int keyCount, byte[]... keys) {
    ...
    // ------------------------------------------------------------------------
    // 上面这部分是原始的代码，根据槽值进行确定批量操作是否允许；
    // 下面这部分代码改成根据jedisPool对象来确定，是否允许批量操作。---- TMD redis协议一定要相同的slot
    // ------------------------------------------------------------------------
    /*
    if (keys.length > 1) {
        int slot = JedisClusterCRC16.getSlot(keys[0]);
        JedisPool pool = connectionHandler.getJedisPoolFromSlot(slot);
        for (int i = 1; i < keyCount; i++) {
          int nextSlot = JedisClusterCRC16.getSlot(keys[i]);
          JedisPool nextPool = connectionHandler.getJedisPoolFromSlot(nextSlot);
          if (pool != nextPool) {
            throw new JedisClusterException("No way to dispatch this command to Redis Cluster "
                + "because keys have different slots.");
          }
