...
<body class="skin-blue sidebar-mini">
		...
		<div class="content-wrapper">
			...
			<section class="content">
				<div class="row">
								...
								<table class="table table-bordered table-hover">
									<tr>
										...
										<input id="title" class="form-control"
											name="title" value="{{block.title}}" placeholder="请输入块显示的名称">
										</td>
									</tr>
									<tr>
										...
										<td>
											<select class="form-control" id="indexBlockType" name="type" onChange="">
												<option value="newsBlock" {{if block.type == 'newsBlock'}}selected{{/if}}>资讯</option>
												<option value="store" {{if block.type == 'store'}}selected{{/if}}>餐厅</option>
												<option value="event" {{if block.type == 'event'}}selected{{/if}}>活动</option>
											</select>
										</td>
									</tr>
									<tr>
										...
										<td>
											<select class="form-control" id="indexActionType" name="action" onChange="">
												<option value="newsIndex" {{if block.action == 'newsIndex'}}selected{{/if}}>推荐列表(资讯)</option>
												<option value="orderIndex" {{if block.action == 'orderIndex'}}selected{{/if}}>订餐首页</option>
												<option value="event" {{if block.action == 'event'}}selected{{/if}}>活动列表</option>
											</select>
										</td>
									</tr>
									<tr>
										<th>置顶ID</th>
										<td>
											<input id="itemIds" class="form-control"
											name="itemIds" value="{{block.itemIds | stringArrayJoin}}" placeholder="例子：30001,30001">
										</td>
									</tr>
								</table>
					...
					<script id="listDivTpl_Add" type="text/html">
						<div class="box" id="block_{{i}}">
							<!-- /.box-header -->
							<div class="box-body">
								<table class="table table-bordered table-hover">
							...
							<div class="box-body">
								<input type="button" value="添加块" class="btn btn-primary"
									...
									class="btn btn-success" onclick="return editIdIndex()">
							</div>
				</div>
			</section>
			<!-- /.content -->
	...
	<script type="text/javascript">
		...
		function loadIndexBlock(){
			...
			requestNode('/user/admin/getindex?vid=${vid}', 'get', {}, function(data){
				var html = template('listDivTpl', {list:data});
				// 模板要一起初始化,不然后面调用会报错
				template('listDivTpl_Add',{i:0});
				document.getElementById('listDiv').innerHTML = html;
				block_i = block_i + data.length;
			});
		}
		
		function addBlock(){
			var html_2 = template('listDivTpl_Add', {i:block_i});
			$('#listDiv').append(html_2);
			block_i = block_i + 1;
		}
		...
		function editIdIndex(){
			...
			var langCode = '${lang.code}';
			var vid = '${vid}';
			requestNode('/user/admin/editindex', 'post', {vid:vid, blockArray:JSON.stringify(blockArray)}, function(data){
