...
<body class="skin-blue sidebar-mini">
										...
										<table id="listTable" class="table table-bordered table-hover">
											...
											<tr>
												...
												<td>
													...
													{{/if}}
													{{if value.status == 2}}
													<a href="javascript:publishFormal({{value.vid}});" class="btn btn-success">刷新</a>
													{{/if}}
												</td>
											</tr>
											{{/each}}
	...
	<script type="text/javascript">
		function publishTest(vid) {
			...
			BootstrapDialog.confirm({
				...
				callback : function(result) {
					if (result) {
						requestNode('/user/admin/publishindex', 'post', {
							vid : vid,
							cityCode : cityCode,
							langCode : langCode
						}, function(data) {
		...
		function add() {
			...
			window.location.href = '/admin/id/index/add.xhtm?cityCode='+cityCode+'&langCode='+langCode;
		}
		...
		function edit(vid) {
			...
			window.location.href = '/admin/id/index/edit.xhtm?vid='+vid+'&cityCode='+cityCode+'&langCode='+langCode;
		}
		...
		function show(vid) {
			...
			window.location.href = '/admin/id/index/show.xhtm?vid='+vid+'&cityCode='+cityCode+'&langCode='+langCode;
		}
		...
		function del(vid) {
			BootstrapDialog.confirm({
				...
				callback : function(result) {
					if (result) {
						requestNode('/user/admin/deleteindex', 'post', {
							vid : vid
						}, function(data) {
							search();
						})
		...
		function search() {
			...
			$('#listTable').artPaginate({
				// 获取数据的地址
				url : "https://napi.mazing.com/user/admin/getindexlist",
				// 模版ID
				tpl : 'listTableTpl',
