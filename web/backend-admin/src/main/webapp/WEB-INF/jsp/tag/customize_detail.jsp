...
<body class="skin-blue sidebar-mini">
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<section class="content-header">
				<a href="/admin/tag/customizeTag.xhtm">返回列表</a>
				<div class="row">
				<div class="col-xs-12">
					<p class="text-left" id="cityCodeTitle"></p>
				</div>
				</div>
			</section>
					...
					<ul id="storeTab" class="nav nav-tabs">
						<li class="active">
							<a href="tagTab" data-toggle="tab">
								<font id="name" color="red"></font>标签列表
							</a>
						</li>
					</ul>
	...
	function loadStoreTag() {
		var customizeId = $.getUrlParam("customizeId");
		if (customizeId == null || customizeId == '' || customizeId == '0') {
			MyDialog.alert('请选择一个配置进行操作');
			return;
		}
		$.getJSON("/admin/tag/listCustomizeTag", {'customizeId':customizeId}, function(json) {
			if (!json.success) {
	...
	function save() {
		...
		if (customizeId == null || customizeId == '' || customizeId == '0') {
			...
			if (values.length > 0) {
				...
				$.post('/admin/tag/updateCustomizeTag.do', {
					'customizeId' : customizeId,
					'tagIds' : tagIds
				}, function(json) {
					MyDialog.alert(json.message, function(){
						window.location.href = '/admin/tag/customizeTag.xhtm';
					});
				}, 'json');
			} else {
