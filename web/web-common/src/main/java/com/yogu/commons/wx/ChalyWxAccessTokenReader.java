...
public class ChalyWxAccessTokenReader {
	...
	private static String cl_appid = null;

	private static String cl_appsecret = null;
	...
	public static Map<String, String> readAccessToken() {
		...
		if (StringUtils.isBlank(accessToken)) {
			...
			accessToken = readChalyAccessToken();
	...
	public static void readConfig() {
		...
		cl_appid = (String) EncryptionPropertyPlaceholderConfigurer.getConfig().get("cl_appid");
		cl_appsecret = (String) EncryptionPropertyPlaceholderConfigurer.getConfig().get("cl_appsecret");
	}
	 ...
	 */
	private static String readChalyAccessToken() {

		RedisLock lock = new RedisLock(redis, "CLWxAccessTokenKey");
		String wxAccessToken = "";
		if (lock.lock()) {

			wxAccessToken = WxAccessTokenRemoteReader.readAccessToken(WxAccessTokenRemoteReader.chaly, cl_appid);

			if (StringUtils.isBlank(wxAccessToken))
				logger.error("chaly#wx#accessToken | 通过chaly接口获取token 出错 | wxAccessToken: {}", wxAccessToken);
			else
				// 缓存，并设置过期时间
