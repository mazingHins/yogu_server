<?xml version="1.0" encoding="UTF-8" standalone="yes" ?>
<?mso-application progid="Excel.Sheet" ?>
	<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:o="urn:schemas-microsoft-com:office:office"
	xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"
	xmlns:html="http://www.w3.org/TR/REC-html40" xmlns:dt="uuid:C2F41010-65B3-11d1-A29F-00AA00C14882">
		<DocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
			<LastAuthor>
				Microsoft Office 用户
			</LastAuthor>
			<Revision>
				1
			</Revision>
			<LastPrinted>
				2016-10-31T03:18:07Z
			</LastPrinted>
			<Created>
				2016-10-24T09:27:41Z
			</Created>
			<LastSaved>
				2016-11-21T03:59:24Z
			</LastSaved>
		</DocumentProperties>
		<CustomDocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
			<KSOProductBuildVer dt:dt="string">
				2052-10.1.0.6028
			</KSOProductBuildVer>
		</CustomDocumentProperties>
		<OfficeDocumentSettings xmlns="urn:schemas-microsoft-com:office:office">
			<Colors>
				<Color>
					<Index>
						2
					</Index>
					<RGB>
						#DD0806
					</RGB>
				</Color>
				<Color>
					<Index>
						3
					</Index>
					<RGB>
						#1FB714
					</RGB>
				</Color>
				<Color>
					<Index>
						4
					</Index>
					<RGB>
						#0000D4
					</RGB>
				</Color>
				<Color>
					<Index>
						5
					</Index>
					<RGB>
						#FCF305
					</RGB>
				</Color>
				<Color>
					<Index>
						6
					</Index>
					<RGB>
						#F20884
					</RGB>
				</Color>
				<Color>
					<Index>
						7
					</Index>
					<RGB>
						#00ABEA
					</RGB>
				</Color>
				<Color>
					<Index>
						8
					</Index>
					<RGB>
						#900000
					</RGB>
				</Color>
				<Color>
					<Index>
						9
					</Index>
					<RGB>
						#006411
					</RGB>
				</Color>
				<Color>
					<Index>
						10
					</Index>
					<RGB>
						#000090
					</RGB>
				</Color>
				<Color>
					<Index>
						11
					</Index>
					<RGB>
						#90713A
					</RGB>
				</Color>
				<Color>
					<Index>
						12
					</Index>
					<RGB>
						#4600A5
					</RGB>
				</Color>
				<Color>
					<Index>
						16
					</Index>
					<RGB>
						#63AAFE
					</RGB>
				</Color>
				<Color>
					<Index>
						17
					</Index>
					<RGB>
						#DD2D32
					</RGB>
				</Color>
				<Color>
					<Index>
						18
					</Index>
					<RGB>
						#FFF58C
					</RGB>
				</Color>
				<Color>
					<Index>
						19
					</Index>
					<RGB>
						#4EE257
					</RGB>
				</Color>
				<Color>
					<Index>
						20
					</Index>
					<RGB>
						#6711FF
					</RGB>
				</Color>
				<Color>
					<Index>
						21
					</Index>
					<RGB>
						#FEA746
					</RGB>
				</Color>
				<Color>
					<Index>
						22
					</Index>
					<RGB>
						#865357
					</RGB>
				</Color>
				<Color>
					<Index>
						23
					</Index>
					<RGB>
						#A2BD90
					</RGB>
				</Color>
				<Color>
					<Index>
						24
					</Index>
					<RGB>
						#63AAFE
					</RGB>
				</Color>
				<Color>
					<Index>
						25
					</Index>
					<RGB>
						#DD2D32
					</RGB>
				</Color>
				<Color>
					<Index>
						26
					</Index>
					<RGB>
						#FFF58C
					</RGB>
				</Color>
				<Color>
					<Index>
						27
					</Index>
					<RGB>
						#4EE257
					</RGB>
				</Color>
				<Color>
					<Index>
						28
					</Index>
					<RGB>
						#6711FF
					</RGB>
				</Color>
				<Color>
					<Index>
						29
					</Index>
					<RGB>
						#FEA746
					</RGB>
				</Color>
				<Color>
					<Index>
						30
					</Index>
					<RGB>
						#865357
					</RGB>
				</Color>
				<Color>
					<Index>
						31
					</Index>
					<RGB>
						#A2BD90
					</RGB>
				</Color>
			</Colors>
		</OfficeDocumentSettings>
		<ExcelWorkbook xmlns="urn:schemas-microsoft-com:office:excel">
			<WindowWidth>
				28695
			</WindowWidth>
			<WindowHeight>
				14430
			</WindowHeight>
			<ProtectStructure>
				False
			</ProtectStructure>
			<ProtectWindows>
				False
			</ProtectWindows>
		</ExcelWorkbook>
		<Styles>
			<Style ss:ID="s44" ss:Name="40% - 强调文字颜色 5">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#B4C6E7" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s37" ss:Name="20% - 强调文字颜色 2">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#FCE4D6" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s12" ss:Name="已访问的超链接">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="11" ss:Color="#800080"
				ss:Underline="Single" />
			</Style>
			<Style ss:ID="s8" ss:Name="千位分隔">
				<NumberFormat ss:Format="_(* #,##0.00_);_(* \(#,##0.00\);_(* &quot;-&quot;??_);_(@_)"
				/>
			</Style>
			<Style ss:ID="s43" ss:Name="强调文字颜色 5">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#4472C4" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s36" ss:Name="40% - 强调文字颜色 1">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#BDD7EE" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s5" ss:Name="千位分隔[0]">
				<NumberFormat ss:Format="_(&quot;$&quot;* #,##0_);_(&quot;$&quot;* \(#,##0\);_(&quot;$&quot;* &quot;-&quot;_);_(@_)"
				/>
			</Style>
			<Style ss:ID="s33" ss:Name="20% - 强调文字颜色 5">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#D9E1F2" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s28" ss:Name="强调文字颜色 2">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#ED7D31" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s10" ss:Name="超链接">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="11" ss:Color="#0000FF"
				ss:Underline="Single" />
			</Style>
			<Style ss:ID="s4" ss:Name="货币">
				<NumberFormat ss:Format="_(&quot;$&quot;* #,##0.00_);_(&quot;$&quot;* \(#,##0.00\);_(&quot;$&quot;* &quot;-&quot;??_);_(@_)"
				/>
			</Style>
			<Style ss:ID="s21" ss:Name="60% - 强调文字颜色 1">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#9BC2E6" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s1" ss:Name="货币[0]">
				<NumberFormat ss:Format="_(* #,##0_);_(* \(#,##0\);_(* &quot;-&quot;_);_(@_)"
				/>
			</Style>
			<Style ss:ID="s45" ss:Name="60% - 强调文字颜色 5">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#8EA9DB" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s40" ss:Name="强调文字颜色 4">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#FFC000" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s38" ss:Name="40% - 强调文字颜色 2">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#F8CBAD" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s13" ss:Name="注释">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"
					/>
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"
					/>
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"
					/>
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#B2B2B2"
					/>
				</Borders>
				<Interior ss:Color="#FFFFCC" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="Default" ss:Name="Normal">
				<Alignment ss:Vertical="Bottom" />
				<Borders/>
				<Font ss:FontName="Arial" x:CharSet="0" ss:Size="10" />
				<Interior/>
				<NumberFormat/>
				<Protection/>
			</Style>
			<Style ss:ID="s6" ss:Name="40% - 强调文字颜色 3">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#DBDBDB" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s2" ss:Name="20% - 强调文字颜色 3">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#EDEDED" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s3" ss:Name="输入">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#3F3F76"
				/>
				<Interior ss:Color="#FFCC99" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s7" ss:Name="差">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#9C0006"
				/>
				<Interior ss:Color="#FFC7CE" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s9" ss:Name="60% - 强调文字颜色 3">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#C9C9C9" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s42" ss:Name="40% - 强调文字颜色 4">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#FFE699" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s27" ss:Name="20% - 强调文字颜色 6">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#E2EFDA" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s17" ss:Name="标题">
				<Font ss:FontName="DengXian Light" x:CharSet="136" ss:Size="18" ss:Color="#44546A"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s11" ss:Name="百分比">
				<NumberFormat ss:Format="0%" />
			</Style>
			<Style ss:ID="s14" ss:Name="60% - 强调文字颜色 2">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#F4B084" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s15" ss:Name="标题 4">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="11" ss:Color="#44546A"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s46" ss:Name="强调文字颜色 6">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#70AD47" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s41" ss:Name="20% - 强调文字颜色 4">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#FFF2CC" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s16" ss:Name="警告文本">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FF0000"
				/>
			</Style>
			<Style ss:ID="s23" ss:Name="60% - 强调文字颜色 4">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#FFD966" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s18" ss:Name="解释性文本">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#7F7F7F"
				ss:Italic="1" />
			</Style>
			<Style ss:ID="s19" ss:Name="标题 1">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="3" ss:Color="#5B9BD5"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="15" ss:Color="#44546A"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s20" ss:Name="标题 2">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="3" ss:Color="#ACCCEA"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="13" ss:Color="#44546A"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s22" ss:Name="标题 3">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="2" ss:Color="#9BC2E6"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="11" ss:Color="#44546A"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s24" ss:Name="输出">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#3F3F3F"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#3F3F3F"
				ss:Bold="1" />
				<Interior ss:Color="#F2F2F2" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s25" ss:Name="计算">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#7F7F7F"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FA7D00"
				ss:Bold="1" />
				<Interior ss:Color="#F2F2F2" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s26" ss:Name="检查单元格">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Left" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Right" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"
					/>
					<Border ss:Position="Top" ss:LineStyle="Double" ss:Weight="3" ss:Color="#3F3F3F"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				ss:Bold="1" />
				<Interior ss:Color="#A5A5A5" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s29" ss:Name="链接单元格">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#FF8001"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FA7D00"
				/>
			</Style>
			<Style ss:ID="s30" ss:Name="汇总">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Double" ss:Weight="3" ss:Color="#5B9BD5"
					/>
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" ss:Color="#5B9BD5"
					/>
				</Borders>
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				ss:Bold="1" />
			</Style>
			<Style ss:ID="s31" ss:Name="好">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#006100"
				/>
				<Interior ss:Color="#C6EFCE" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s32" ss:Name="适中">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#9C6500"
				/>
				<Interior ss:Color="#FFEB9C" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s34" ss:Name="强调文字颜色 1">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#5B9BD5" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s35" ss:Name="20% - 强调文字颜色 1">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#DDEBF7" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s39" ss:Name="强调文字颜色 3">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#A5A5A5" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s47" ss:Name="40% - 强调文字颜色 6">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#000000"
				/>
				<Interior ss:Color="#C6E0B4" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s48" ss:Name="60% - 强调文字颜色 6">
				<Font ss:FontName="DengXian" x:CharSet="136" ss:Size="12" ss:Color="#FFFFFF"
				/>
				<Interior ss:Color="#A9D08E" ss:Pattern="Solid" />
			</Style>
			<Style ss:ID="s49">
				<Alignment ss:Vertical="Bottom" />
				<Borders/>
				<Font ss:FontName="Arial" x:CharSet="0" ss:Size="10" />
				<Interior/>
				<NumberFormat/>
				<Protection/>
			</Style>
			<Style ss:ID="s50">
				<Alignment ss:Horizontal="Center" />
			</Style>
			<Style ss:ID="s51" />
			<Style ss:ID="s52">
				<Alignment ss:Horizontal="Center" />
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
				<Font ss:FontName="微软雅黑" x:CharSet="136" ss:Size="12" />
			</Style>
			<Style ss:ID="s53">
				<Alignment ss:Horizontal="Center" ss:Vertical="Bottom" />
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
				<Font ss:FontName="宋体" x:CharSet="0" ss:Size="10" />
			</Style>
			<Style ss:ID="s54">
				<Alignment ss:Horizontal="Right" ss:Vertical="Bottom" />
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
			</Style>
			<Style ss:ID="s55">
				<Alignment ss:Horizontal="Center" ss:Vertical="Bottom" />
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
				<Font ss:FontName="Arial" x:CharSet="0" ss:Size="12" />
				<NumberFormat ss:Format="0_ " />
			</Style>
			<Style ss:ID="s56">
				<Alignment ss:Horizontal="Center" ss:Vertical="Bottom" />
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
				<Font ss:FontName="Arial" x:CharSet="0" ss:Size="12" />
				<NumberFormat ss:Format="@" />
			</Style>
			<Style ss:ID="s57">
				<Borders>
					<Border ss:Position="Bottom" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Left" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Right" ss:LineStyle="Continuous" ss:Weight="1" />
					<Border ss:Position="Top" ss:LineStyle="Continuous" ss:Weight="1" />
				</Borders>
				<Font ss:FontName="Arial" x:CharSet="0" ss:Size="12" />
				<NumberFormat ss:Format="0.00_ " />
			</Style>
		</Styles>
		
		<#if onlineDataMap ?? && onlineDataMap.online == 1>
			<Worksheet ss:Name="在线订餐">
				<Table ss:ExpandedColumnCount="18" ss:ExpandedRowCount="11" x:FullColumns="1"
				x:FullRows="1" ss:DefaultColumnWidth="60.4" ss:DefaultRowHeight="12.75">
					<Column ss:Index="1" ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="96"/>
					<#if isShowPhone == true>
						<Column ss:Index="2" ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="93.75"/>
					</#if>
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="124.5" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="122.25" />
					<Column ss:StyleID="s51" ss:Width="81.4" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="102.75" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="97.15" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="102" />
					<!-- 显示顺丰配送列-->
					<#if onlineDataMap.isSfexpress == 1>
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="92.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="98.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="122.25" />
					<Column ss:StyleID="s51" ss:Width="102.4" />
					</#if>
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="92.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="98.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="122.25" />
					<Column ss:StyleID="s51" ss:Width="102.4" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="97.15" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="84" />
					<Column ss:StyleID="s51" ss:Width="70.9" />
					<Column ss:Index="18" ss:StyleID="s51" ss:Width="49.9" />
					<Row ss:StyleID="s50" ss:Height="27">
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								餐厅名称
							</Data>
						</Cell>
						<#if isShowPhone == true>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									用户帐号
								</Data>
							</Cell>
						</#if>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								顾客昵称
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								米星订单号
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								创建时间
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								支付方式
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								订单金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								优惠券金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								实付金额（元）
							</Data>
						</Cell>
						<#if onlineDataMap.isSfexpress == 1>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									顺丰专送顾客承担金额（元）
								</Data>
							</Cell>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									顺丰专送餐厅承担金额（元）
								</Data>
							</Cell>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									顺丰专送米星补助金额（元）
								</Data>
							</Cell>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									顺丰专送总金额（元）
								</Data>
							</Cell>
						</#if>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								实收金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								优惠金额承担方
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								订单完成时间
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								备注
							</Data>
						</Cell>
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
					</Row>
					<#if onlineDataMap.orderExcelList?? && (onlineDataMap.orderExcelList?size > 0)>
						<#list onlineDataMap.orderExcelList as excel>
							<Row ss:Height="16.5">
								<Cell ss:StyleID="s53">
									<Data ss:Type="String">${(excel.storeName)!''}</Data>
								</Cell>
								<#if isShowPhone == true>
									<Cell ss:StyleID="s54">
										<Data ss:Type="String">${(excel.passport)!''}</Data>
									</Cell>
								</#if>
								<Cell ss:StyleID="s54">
									<Data ss:Type="String">${(excel.userName)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s55">
									<Data ss:Type="Number">
										${(excel.orderNo)!''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.createTime)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s53">
									<Data ss:Type="String"><#if excel.payType == 1>在线支付</#if><#if excel.payType == 3>米星pay</#if></Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.totalFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.discountFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.actualFee)/100 !''}
									</Data>
								</Cell>
								<#if onlineDataMap.isSfexpress == 1>
									<Cell ss:StyleID="s54">
										<Data ss:Type="Number">
											${(excel.userDeliveryFee)/100 !''}
										</Data>
									</Cell>
									<Cell ss:StyleID="s54">
										<Data ss:Type="Number">
											${(excel.storeDeliveryFee)/100 !''}
										</Data>
									</Cell>
									<Cell ss:StyleID="s54">
										<Data ss:Type="Number">
											${(excel.mzDeliveryFee)/100 !''}
										</Data>
									</Cell>
									<Cell ss:StyleID="s54">
										<Data ss:Type="Number">
											${(excel.expressNeedFee)/100 !''}
										</Data>
									</Cell>
								</#if>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.receivedFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s53">
									<Data ss:Type="String"><#if excel.couponBearType == 1>米星</#if><#if excel.couponBearType == 2>商家</#if></Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.finishTime)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.remark)!''}</Data>
								</Cell>
							</Row>
						</#list>
					</#if>
				</Table>
				<WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
					<PageSetup>
						<Header x:Margin="0.3" />
						<Footer x:Margin="0.3" />
					</PageSetup>
					<Selected/>
					<TopRowVisible>
						0
					</TopRowVisible>
					<LeftColumnVisible>
						0
					</LeftColumnVisible>
					<Panes>
						<Pane>
							<Number>
								3
							</Number>
							<ActiveRow>
								9
							</ActiveRow>
							<ActiveCol>
								7
							</ActiveCol>
							<RangeSelection>
								R10C8
							</RangeSelection>
						</Pane>
					</Panes>
					<ProtectObjects>
						False
					</ProtectObjects>
					<ProtectScenarios>
						False
					</ProtectScenarios>
				</WorksheetOptions>
			</Worksheet>
		</#if>
		
		<#if mazingDataMap ?? && mazingDataMap.mazingPay == 3>
			<Worksheet ss:Name="米星pay">
				<Table ss:ExpandedColumnCount="18" ss:ExpandedRowCount="11" x:FullColumns="1"
				x:FullRows="1" ss:DefaultColumnWidth="60.4" ss:DefaultRowHeight="12.75">
					<Column ss:Index="1" ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="96"/>
					<#if isShowPhone == true>
						<Column ss:Index="2" ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="93.75"/>
					</#if>
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="93.75"/>
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="124.5" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="122.25" />
					<Column ss:StyleID="s51" ss:Width="81.4" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="102.75" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="97.15" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="102" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="92.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="98.25" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="122.25" />
					<Column ss:StyleID="s51" ss:Width="102.4" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="97.15" />
					<Column ss:StyleID="s51" ss:AutoFitWidth="0" ss:Width="84" />
					<Column ss:StyleID="s51" ss:Width="70.9" />
					<Column ss:Index="18" ss:StyleID="s51" ss:Width="49.9" />
					<Row ss:StyleID="s50" ss:Height="27">
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								餐厅名称
							</Data>
						</Cell>
						<#if isShowPhone == true>
							<Cell ss:StyleID="s52">
								<Data ss:Type="String">
									用户帐号
								</Data>
							</Cell>
						</#if>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								顾客昵称
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								米星订单号
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								创建时间
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								支付方式
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								订单金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								优惠券金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								实付金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								实收金额（元）
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								优惠金额承担方
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								订单完成时间
							</Data>
						</Cell>
						<Cell ss:StyleID="s52">
							<Data ss:Type="String">
								备注
							</Data>
						</Cell>
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
						<Cell ss:StyleID="Default" />
					</Row>
					<#if mazingDataMap.mazingPayList?? && (mazingDataMap.mazingPayList?size > 0)>
						<#list mazingDataMap.mazingPayList as excel>
							<Row ss:Height="16.5">
								<Cell ss:StyleID="s53">
									<Data ss:Type="String">${(excel.storeName)!''}</Data>
								</Cell>
								<#if isShowPhone == true>
									<Cell ss:StyleID="s54">
										<Data ss:Type="String">${(excel.passport)!''}</Data>
									</Cell>
								</#if>
								<Cell ss:StyleID="s54">
									<Data ss:Type="String">${(excel.userName)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s55">
									<Data ss:Type="Number">
										${(excel.orderNo)!''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.createTime)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s53">
									<Data ss:Type="String"><#if excel.payType == 1>在线支付</#if><#if excel.payType == 3>米星pay</#if></Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.totalFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.discountFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.actualFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s57">
									<Data ss:Type="Number">
										${(excel.receivedFee)/100 !''}
									</Data>
								</Cell>
								<Cell ss:StyleID="s53">
									<Data ss:Type="String"><#if excel.couponBearType == 1>米星</#if><#if excel.couponBearType == 2>商家</#if></Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.finishTime)!''}</Data>
								</Cell>
								<Cell ss:StyleID="s56">
									<Data ss:Type="String">${(excel.remark)!''}</Data>
								</Cell>
							</Row>
						</#list>
					</#if>
				</Table>
				<WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
					<PageSetup>
						<Header x:Margin="0.3" />
						<Footer x:Margin="0.3" />
					</PageSetup>
					<Selected/>
					<TopRowVisible>
						0
					</TopRowVisible>
					<LeftColumnVisible>
						0
					</LeftColumnVisible>
					<Panes>
						<Pane>
							<Number>
								3
							</Number>
							<ActiveRow>
								9
							</ActiveRow>
							<ActiveCol>
								7
							</ActiveCol>
							<RangeSelection>
								R10C8
							</RangeSelection>
						</Pane>
					</Panes>
					<ProtectObjects>
						False
					</ProtectObjects>
					<ProtectScenarios>
						False
					</ProtectScenarios>
				</WorksheetOptions>
			</Worksheet>
		</#if>
	</Workbook>